...
program sum_kernels
  ...
  real(kind=CUSTOM_REAL), allocatable, dimension(:) :: kernel1, kernel2, kernel3, kernel4, kernel5, kernel6, kernel7
  ...
  do j=1,nline
     read(100,*) xcoord(j), zcoord(j), kernel1(j),  kernel2(j), kernel3(j), kernel4(j), kernel5(j), kernel6(j), kernel7(j)
  enddo
  close(100)
...
subroutine get_number_lines(filename, nline)
  ...
  real(kind=CUSTOM_REAL) :: dummy1, dummy2, dummy3, dummy4, dummy5, dummy6, dummy7, dummy8, dummy9
  ...
  do j=1,MAX_NUMBER_LINES
     read(100,*,iostat=ios) dummy1, dummy2, dummy3, dummy4, dummy5, dummy6, dummy7, dummy8, dummy9
     if (ios /= 0) exit
     nline=nline+1
  enddo
  close(100)
